...
| `PLUGIN_BITBUCKET_TOKEN` | Required on **every** event, not just `pull_request` — it authenticates the branch fetch used for tag resolution, regardless of event. Missing it on `pull_request` fails the run outright (no PR description); missing it on `manual`/`push` doesn't crash, but the fetch 401s and every component is silently mis-detected as a first release. See [§6](#6-triggering-events--manual-pull_request-and-push-merge). |
