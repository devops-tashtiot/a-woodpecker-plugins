    ...
    def _match_line(current_line):
        ...
        for p in parsers:
            ...
            if pattern_match and current_line[pattern_match.end():pattern_match.end()+1] == '[':
                bracket_match = bracket_re.search(current_line, pattern_match.end())
        ...
        if bracket_match:
            ...
            if failed:
                def _level_reason(loc):
                    ...
                    return (f"'{loc}' - wrong depth: location has {actual} path segment(s), "
                            f"but PLUGIN_CHANGELOG_LEVEL={changelog_level} requires exactly {changelog_level} slash(es)")
            ...
            loc_display = ", ".join([f"'{loc}'" if loc else "root" for loc in locations])
...
def _print_cliff_rules(parsers, bump_cfg, toml_path=None):
    ...
    print("    type is either skipped or treated as unreleaseable, depending on your cliff.toml")
...
def _print_location_commits(location_to_commits):
    ...
    for loc in sorted(location_to_commits):
        ...
        for commit in sorted(location_to_commits[loc]):
            # \n inside a commit string will render as an actual newline with proper indentation
...
def release():
    ...
    if not os.getenv("PLUGIN_CHANGELOG_LEVEL"):
        missing.append(
            ...
            "    Level 2 -> nested:    feat[plugins/docker][...]  Example: PLUGIN_CHANGELOG_LEVEL=2"
        )
    ...
    for location in sorted(location_to_commits):
        ...
        print(f">>> [INFO] Latest tag (base):    " + 
              (f"'{latest_tag}'" if latest_tag else f"(none - first release -> will use '{first_tag}')"))
